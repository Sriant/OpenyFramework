 ...
 * state.
 */
#define RING_BUFFER_MAX_SIZE        0x00008000U
...
#define RING_BUF_DECLARE(name, size8)                                                              \
    ...
    struct ring_buf name = {.buffer = _ring_buffer_data_##name, .size = size8}
...
#define RING_BUF_ITEM_DECLARE(name, size32)                                                        \
    ...
    struct ring_buf name = {.buffer = (uint8_t *)_ring_buffer_data_##name, .size = 4 * (size32)}
 ...
 * @param buf Address of ring buffer.
 */
static inline void ring_buf_reset(struct ring_buf *buf) { ring_buf_internal_reset(buf, 0); }
 ...
 */
static inline uint32_t ring_buf_capacity_get(struct ring_buf *buf) { return buf->size; }
